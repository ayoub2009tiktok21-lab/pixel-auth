...
def main() -> None:
    ...
    # Unset GCP project vars to prevent PTB/Google SDK from attempting Datastore persistence on unconfigured projects
    os.environ.pop("GOOGLE_CLOUD_PROJECT", None)
    os.environ.pop("GCP_PROJECT", None)

    _log_startup_diagnostics()

    # Disable default external JobQueue persistence to avoid 404 Datastore errors on Replit/Cloud environments
    ...
    # Periodic job: purge expired sessions every 5 minutes if job_queue is active
    if app.job_queue:
        try:
            ...
            logger.warning("JobQueue initialization skipped: %s", err)
